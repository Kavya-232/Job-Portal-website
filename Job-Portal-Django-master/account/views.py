...
def employee_edit_profile(request, id):
    ...
    print(f"User ID: {request.user.id}, Requested ID: {id}")  

    if request.user.id != id:
        return redirect("accounts:forbidden")  # Ensure a proper forbidden route exists

    form = EmployeeProfileEditForm(request.POST or None, instance=user)
    if form.is_valid():
        form.save()
        messages.success(request, 'Your Profile Was Successfully Updated!')
        return redirect(reverse("accounts:edit-profile", kwargs={'id': user.id}))
