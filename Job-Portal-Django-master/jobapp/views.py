...
def dashboard_view(request):
    ...
    if user_role == 'employer':
        jobs = Job.objects.filter(user=request.user.id).annotate(
            applicant_count=Count('applicant')
        )
        total_applicants = {job.id: job.applicant_count for job in jobs}
...
from django.shortcuts import render, get_object_or_404, redirect
from .models import Job, Apply  # Assuming Apply is the model for job applications
...
def apply_job(request, job_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Create application object, associate with job and user
            application = form.save(commit=False)
            application.job = job
            application.user = request.user  # Associate application with logged-in user
            application.save()
            
            # Add success message
            messages.success(request, 'Your application has been submitted successfully.')
            
            # Redirect to job list or any other page after success
            return redirect('jobapp:job_list')  # Adjust to your desired URL

    else:
        # Create an empty form if the request is GET
        form = ApplicationForm()

    return render(request, 'jobapp/index.html', {'form': form, 'job': job})
